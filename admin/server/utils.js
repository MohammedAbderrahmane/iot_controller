...
async function registerIoTObjectOnFognode(iotObject,fognode) {
  return sendCoapRequestAsync({
    host : fognode.ipAddress ,
    method: 'POST',
    confirmable: true,
    pathname: '/objects',
    port:fognode.port
  ...
  return new Promise((resolve, reject) => {
    ...
    request.on('response', (res) => {
      ...
      res.on('end', () => {
       ...
       res.on('error', (err) => { // Handle errors during response stream reading
    ...
    if (options.confirmable) {
      request.on('timeout', (err) => {
        ...
        reject(new Error(`CoAP request timed out after ${request.options.ackTimeout * request.options.ackRandomFactor * (1 << request.options.retransmit)}s`));
      });
    }
...
module.exports = { getAuthorities , registerIoTObjectOnFognode };
